...
//  AdventOfCode2019
//
//  Created by Jake Sawyer on 12/2/19.
...
class Day3 {
    ...
    var input: (left: [Command], right: [Command]) {
        let wires = rawInput.components(separatedBy: "\n")
        return (left: wires[0].components(separatedBy: ",").map { Command(rawValue: $0) }, right: wires[1].components(separatedBy: ",").map { Command(rawValue: $0) })
    }
    ...
    struct Command {
        ...
        init(rawValue: String) {
            amount = Int(rawValue.dropFirst())!
            switch rawValue.first!.lowercased() {
            case "r":
                delta.x = 1
            case "u":
                delta.y = 1
            ...
            case "l":
                delta.x = -1
            default:
                fatalError("Unknown direction: \(rawValue)")
            }
        }
    }
    
    func solution1() -> Int {
        var leftPositions = Set<Point>()
        var rightPositions = Set<Point>()
        var distances = [Point: Int]()
        var closestDistance = Int.max
        let (leftWireCommands, rightWireCommands) = input
        var currentLeftPosition = Point.zero
        var currentRightPosition = Point.zero
        
        func compute(index: Int, currentPosition: inout Point, commands: [Command], positions: inout Set<Point>, otherPositions: Set<Point>) {
            guard index < commands.count else { return }
            let command = commands[index]
            for _ in 0 ..< command.amount {
                currentPosition &+= command.delta
                positions.insert(currentPosition)
                if otherPositions.contains(currentPosition) && !distances.keys.contains(currentPosition) {
                    let distance = abs(currentPosition.x) + abs(currentPosition.y)
        ...
        for index in 0 ..< max(leftWireCommands.count, rightWireCommands.count) {
            compute(index: index, currentPosition: &currentLeftPosition, commands: leftWireCommands, positions: &leftPositions, otherPositions: rightPositions)
            compute(index: index, currentPosition: &currentRightPosition, commands: rightWireCommands, positions: &rightPositions, otherPositions: leftPositions)
        }
    ...
    func solution2() -> Int {
        var leftPositions = [Point: Int]()
        var rightPositions = [Point: Int]()
        var leftSteps = 0
        var rightSteps = 0
        var distances = [Point: Int]()
        var closestDistance = Int.max
        ...
        func compute(index: Int, steps: inout Int, currentPosition: inout Point, commands: [Command], positions: inout [Point: Int], otherPositions: [Point: Int]) {
            guard index < commands.count else { return }
            let command = commands[index]
            for _ in 0 ..< command.amount {
                steps += 1
                currentPosition &+= command.delta
                if !positions.keys.contains(currentPosition) {
                    positions[currentPosition] = steps
                }
                if otherPositions.keys.contains(currentPosition) && !distances.keys.contains(currentPosition) {
                    let distance = positions[currentPosition]! + otherPositions[currentPosition]!
                    distances[currentPosition] = distance
                    if distance < closestDistance {
                        closestDistance = distance
                    }
        ...
        for index in 0 ..< max(leftWireCommands.count, rightWireCommands.count) {
            compute(index: index, steps: &leftSteps, currentPosition: &currentLeftPosition, commands: leftWireCommands, positions: &leftPositions, otherPositions: rightPositions)
            compute(index: index, steps: &rightSteps, currentPosition: &currentRightPosition, commands: rightWireCommands, positions: &rightPositions, otherPositions: leftPositions)
        }
