...
class Day5 {
    func solution1() -> Int {
        execute(inputValue: 1, values: input)
    }
    
    func solution2() -> Int {
        execute(inputValue: 5, values: input)
    }
    
    func execute(inputValue: Int, values: [Int]) -> Int {
        var index = 0
        var values = values
        while true {
            ...
            func operate(operation: (Int, Int) -> Int) {
                ...
                let a = modeA == 1 ? values[index + 1] : values[values[index + 1]]
                let b = modeB == 1 ? values[index + 2] : values[values[index + 2]]
                let storeIndex = values[index + 3]
                values[storeIndex] = operation(a, b)
            }
            ...
            func evaluate(operation: (Int, Int) -> Bool) {
                ...
                let b = modeB == 1 ? values[index + 2] : values[values[index + 2]]
                let storeIndex = values[index + 3]
                values[storeIndex] = operation(a, b) ? 1 : 0
            }
            ...
            case 2:
                ...
                index += 4
            case 3:
                let storeIndex = values[index+1]
                values[storeIndex] = inputValue
                index += 2
            case 4:
                let mode = digit(of: opcode, atPlace: 2)
                let value = mode == 0 ? values[values[index+1]] : values[index+1]
                print("Opcode \(opcode): \(value)")
                index += 2
    ...
    lazy var input: [Int] = {
        rawInput.split(separator: ",").map { Int($0)! }
    }()
